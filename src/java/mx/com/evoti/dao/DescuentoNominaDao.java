...
public class DescuentoNominaDao extends ManagerDB implements Serializable {
    ...
    public List<DescuentoNominaDto> extraeDescuentoNominaEmpresasFAAG(Date fechaCatorcena) throws IntegracionException {
        ...
        SQLQuery query = session.createSQLQuery(
                String.format("select cre_fecha_deposito as fechaDeposito,cre_prestamo as prestamo,cre_fecha_primer_pago as fechaPrimerPago,p.pro_siglas as producto,cre_clave as claveCredito,e.cre_est_nombre as statusCredito,u.usu_clave_empleado as claveEmpleado,concat_ws(' ',usu_nombre,usu_paterno,usu_materno) as nombre,m.emp_abreviacion as empresa,sum(a.amo_interes) as interes,sum(a.amo_monto_pago) as pagoTotal\n" +
